...
async def process_chunk(chunk, start_idx, total_records, model_client, termination_condition):
    ...
    prompt = (
        ...
        f"以下為該批次資料:\n{chunk_data}\n\n"
        "請根據以上資料進行分析，並提供完整的食品添加劑分析。"
        "其中請特別注意：\n"
        "  1. 不同食品添加劑；\n"
        "  2. 請 MultimodalWebSurfer 搜尋外部網站，找出不同高度使用的食品添加劑可能對身體造成何種危害，\n"
        "     並將搜尋結果整合進回覆中；\n"
        "  3. 最後請提供具體的建議和相關參考資訊，例如如何減少食品添加劑的使用或是日常生活如何吃得更健康。\n"
...
async def main():
    ...
    # output_file = "all_conversation_log_test.csv"
    # df_log.to_csv(output_file, index=False, encoding="utf-8-sig")
    # print(f"已將所有對話紀錄輸出為 {output_file}")

    # 取得目前的日期和時間，用來命名輸出的 CSV 檔案
    current_datetime = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

    # 確保資料夾存在
    output_folder = "all_conversation_log_test"
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # 設定檔案路徑，將對話紀錄整理成 DataFrame 並存成 CSV
    output_file = os.path.join(output_folder, f"{current_datetime}.csv")
    df_log = pd.DataFrame(all_messages)
