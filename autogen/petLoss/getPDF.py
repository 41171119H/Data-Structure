    ...
    for index, row in df.iterrows():
        if pdf.get_y() + cell_height > pdf.h - pdf.b_margin:
            ...
            if not chinese_font_path:
                return "錯誤：找不到中文字型，請放入 fonts 資料夾中！"
...
def create_table(pdf: FPDF, df: pd.DataFrame):
    ...
    for col in df.columns:
        if "留言" in col or "關鍵詞" in col:
            col_widths.append(available_width * 0.4)  # 留言或關鍵詞給 40%
        else:
            col_widths.append(available_width * 0.6 / (len(df.columns) - 1))  # 其餘平均分 60%

    # 確保欄寬加總為 available_width
    ...
    for index, row in df.iterrows():
        ...
        for i, item in enumerate(row):
            text = str(item)
            lines = len(pdf.multi_cell(col_widths[i], cell_height, text, border=0, align="L", split_only=True))
            row_heights.append(lines * cell_height)
            max_y = max(max_y, y_before + lines * cell_height)
...
1. 判斷該留言的情緒傾向（正向、中立、負向）
2. 擷取留言中出現的關鍵詞（例如：服務、價格、洗澡、環境、客服、商品）
3. 將情緒傾向與關鍵詞整理成統計報表
4. 最後請列出所有出現過的關鍵詞與其出現次數
"""
